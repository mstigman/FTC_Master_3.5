...
public abstract class MechanumTestBot extends LinearOpMode{
    ...
    protected void stopAtGlyph(int nubmer) throws InterruptedException
    {
        int numberAt = 0;
        boolean seen = false;
        while(range.getDistance(DistanceUnit.CM) > 30)
        {
        ...
        while (opModeIsActive() && numberAt < nubmer)
        {
            ...
            telemetry.update();
            if(range.getDistance(DistanceUnit.CM) <= 18 && !seen)
            {
                seen = true;
                numberAt++;
            }
            ...
            {
                if(range.getDistance(DistanceUnit.CM) > 18)
                {
                    seen = false;
                }
            }
    ...
    protected void dumpBlocks(boolean check_left_bumper) throws InterruptedException {
        ...
        if((!check_left_bumper || gamepad1.left_bumper))
        {
            sleep(500);
        }
